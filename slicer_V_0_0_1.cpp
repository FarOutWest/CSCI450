...
public:
  ...
  float distanceToPoint(const v3 &vertex) const {
    return vertex.dotproduct(mNormal) - mDistance;
  }
  ...
  if (!isBinaryFormat) {
    ...
    while (!in.eof()) {
      ...
      if (s0 == "facet") {
        ...
        Triangle t(v3(n0, n1, n2), v3(f0, f1, f2), v3(f3, f4, f5),
                   v3(f6, f7, f8));
        mesh->push_back(t);
      }
    ...
    // Every Face is 50 Bytes: Normal(3*float), Vertices(9*float), 2 Bytes
    // Spacer
    for (size_t i = 0; i < nFaces; ++i) {
      for (size_t j = 0; j < 12; ++j) {
        fread((void *)&v[j], sizeof(float), 1, f);
      }
      fread((void *)&uint16, sizeof(unsigned short), 1,
            f); // spacer between successive faces
      Triangle t(v3(v[0], v[1], v[2]), v3(v[3], v[4], v[5]),
...
int triMeshSlicer(
  ...
  const v3 aabb = mesh->meshAABBSize(); // as the model for it’s 3D axis-aligned
                                        // bounding-box
  const size_t nSlices =
      1 + (int)(aabb.z / sliceSize); // compute number of output slices
  const std::vector<Triangle> &m =
      mesh->getMesh(); // get a const handle to the input mesh
  const float z0 = mesh->getBottomLeftVertex()
                       .z; // find the minimal z coordinate of the model (z0)
  for (size_t i = 0; i < nSlices; ++i) { // start generating slices
    std::vector<LineSegment> linesegs;   // the linesegs vector for each slice
    plane.setDistance(
        z0 +
        (float)i * sliceSize); // position the plane according to slice index
    for (size_t t = 0; t < m.size(); ++t) { // iterate all mesh triangles
      const Triangle &triangle = m[t];      // get a const handle to a triangle
      LineSegment ls;
      if (0 ==
          triangle.intersectPlane(
              plane, ls)) {     // the plane does intersect the triangle
        linesegs.push_back(ls); // push a new Line Segment object to this slice
      }
    }
    slicesWithLineSegments.push_back(
        linesegs); // push this vector to the slices vector
  }
  return 0;
}
...
int exportSingleGIVFormat3D(
  ...
  glm::tvec3<float> eulerAngles(0, 0.0f, 45.0f);
  glm::tquat<float> quaternion = glm::tquat<float>(
      DEG_TO_RAD(eulerAngles)); // This glm func wants values as radians
